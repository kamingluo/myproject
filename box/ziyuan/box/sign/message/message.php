...
$sql    = "select * from box_sign_msg" ; //查询提交到数据库的信息
...
echo "推送完成啦";
$mysql    = "truncate box_sign_msg" ; //清空数据表
...
function temMsg($username,$userformid,$useropenid,$b)
{
    $name = $username;
    $formid = $userformid;
    $temid = 'EaK0Fy-PZtrvFjy3OpcFLVM5MNiIB4fSnNU1_j777SM';
    ...
    $data = array(//这里一定要按照微信给的格式
        ...
        "data"=>array(
            ...
            "keyword3"=>array(
                "value"=>"未签到",
                "color"=>"#FF2D2D"  //应该是不能用大写，但是我懒得改
            ),
            "keyword4"=>array(
                "value"=>"今日新增更多游戏，更多隐藏福利等你来发掘！",
                "color"=>"#173177"
            )
        ),
