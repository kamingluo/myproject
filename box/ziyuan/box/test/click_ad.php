	    ...
	    else
	    {
	       ...
	       if($_GET['master_id']==0||$_GET['master_id']==null){
		       echo "没有师傅";
		        ...
		        $mastersql = "UPDATE box_user SET score = score +'20' , pay_tribute=pay_tribute+'20' WHERE id='$_GET[master_id]'";
                $masterresult = mysql_query($mastersql,$con);
                echo "给师傅加金币成功，，，";

                $mastermysql = "INSERT INTO box_score_record (`id`,`userid`,`score`,`explain`,`create_time`) VALUES ('', '$_GET[master_id]','20','徒弟进贡',now())";
	            $mastermyresult = mysql_query($mastermysql,$con);
	            echo "添加师傅金币数据记录成功，，，";

	           
                $tribute = "UPDATE box_tribute_table SET pay_tribute = pay_tribute +'20'  WHERE master_id='$_GET[master_id]' and openid ='$_GET[openid]'";
