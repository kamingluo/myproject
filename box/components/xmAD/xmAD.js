...
Component({
    ...
    methods: {
      ...
      var xmadonloadtime = wx.getStorageSync("appconfig").xmadonloadtime
      for (var i = 0; i < xmadonloadtime; i++) {　　
        setTimeout(function() {　　　　
          that.triggerEvent('adload')
          //console.log("小盟循环提交加载adload")　　
        }, i * 3000);
      }
      var xmadchance = wx.getStorageSync("appconfig").xmadchance
    setTimeout(function() {
      let datanumber = Math.floor(Math.random() * xmadchance)
